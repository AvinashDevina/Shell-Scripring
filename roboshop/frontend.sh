...
echo -n -e "\t\e[32m3. Cleaning Old Code\e[0m.... \t\t\t"
cd /usr/share/nginx/html && rm -rf *
STATUS_CHECK $?

echo -n -e "\t\e[32m4. Extracting & Moving Frontend Code\e[0m.... \t"
unzip /tmp/frontend.zip &>>$LOG && mv frontend-main/* . &>>$LOG && mv static/* . &>>$LOG
STATUS_CHECK $?

echo -n -e "\t\e[32m5. Cleaning useless Directories\e[0m.... \t\t"
cd /usr/share/nginx/html && rm -rf frontend-main static &>>$LOG
STATUS_CHECK $?

echo -n -e "\t\e[32m6. Updating roboshop.conf\e[0m.... \t\t\t"
